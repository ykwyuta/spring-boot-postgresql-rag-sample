...
class KnowledgeServiceTest {
    ...
    void normalizesFiltersAndBoundsResultLimit() {
        ...
        service.search(" 冷蔵 ", "rule", "北海道", "approved", date, 999);

        verify(mapper).search("冷蔵", "RULE", "北海道", "APPROVED", date, 50);
    }

    @Test
    void normalizesCodeAndBoundsGraphDepth() {
        LocalDate date = LocalDate.of(2026, 9, 17);
        service.related("rule-002", 99, date, 0);

        verify(mapper).findRelated("RULE-002", 3, date, 1);
    }

    @Test
    void rejectsCodeThatCouldChangeSqlOrGraphQueries() {
        assertThatThrownBy(() -> service.get("RULE-002' OR true", LocalDate.now()))
    ...
    void returnsMapperResultsWithoutRewritingEvidence() {
        ...
        when(mapper.search(any(), any(), any(), any(), any(), anyInt())).thenReturn(List.of(item));

        assertThat(service.search("冷蔵", null, null, null, LocalDate.of(2026, 9, 17), 10))
